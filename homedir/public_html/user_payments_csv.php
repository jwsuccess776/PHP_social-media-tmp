...
if (isset($lstMonth) and $lstMonth != '-Choose-' and $lstYear != '-Choose-') {
    ...
    $query="SELECT *, DATE_FORMAT(pay_date,'%d-%M-%Y') AS paydate
        ...
        AND YEAR(pay_date) = '$lstYear'

        AND pay_userid = '$Sess_UserId' AND pay_transid != '0'
...
echo USER_PAYMENTS_CSV_TDATE.";".USER_PAYMENTS_CSV_TCODE.";".USER_PAYMENTS_CSV_TSTATUS.";".USER_PAYMENTS_CSV_AMOUNT;

?>

<?php   while ($sql_array = mysqli_fetch_object($result) ) {?>

<?php echo date($CONST_FORMAT_DATE_SHORT,strtotime($sql_array->paydate));?>;<?php echo $sql_array->pay_transid?>;<?php echo $sql_array->pay_transstatus?>;<?php echo "$CONST_SYMBOL$sql_array->pay_samount\r\n"?>
