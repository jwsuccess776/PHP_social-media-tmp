...
if ($_REQUEST['OK']){
    ...
    foreach (get_payment_params('payflow_api',$payment->pay_service) as $param) {
        $order["$param->psp_name"] = $param->psp_value;
    }

    $p = new PFPro($CONST_PAYFLOW_ROOT, $order["Username"], $order["Password"], $order["Partner"], $order["VendorName"]);

    $order["chargetotal"]   = $payment->pay_samount;
    $order["comments"]      = $payment->pay_message;
    $order["orderdata"]     = $payment->pay_paymentid;
    //Collect customer info
    $addr =sanitizeData($_POST['address1'], 'xss_clean') ; 
    $zip =sanitizeData($_POST['zip'], 'xss_clean') ;  
    $p->setCustomerInfo($addr, $zip);

    $cc =sanitizeData($_POST['cardnumber'], 'xss_clean') ;  
    $exp =sanitizeData($_POST['cardexpmonth'], 'xss_clean') . sanitizeData($_POST['cardexpyear'], 'xss_clean') ; 
    $cvv =sanitizeData($_POST['cvmvalue'], 'xss_clean') ;
    $p->setPaymentInfo($cc, $exp, $cvv, $order["chargetotal"]);

    $cvmvalue =sanitizeData($_POST['cvmvalue'], 'xss_clean') ;  
    if ($cvmvalue) {
...
//    print_r($order);exit;
    ...
    if($p->process() == false)
        $error = "<b>Error:</b>" . $p->getLastMessage();
    else
    {
/*
        if($p->isInternational())
            $p->setSecurityLevel("none", "none");
        else
            $p->setSecurityLevel("medium", "medium");
*/
        $p->setSecurityLevel("none", "none");
/*
        if($p->fraudCheck() == false)
        {
            //AVS and/or CVV checks failed
            if($p->void(false) == false)
                $error = "<b>Error voiding transaction</b>: " . $p->getLastMessage();
            else
                $error = "<b>Declined:</b> AVS or CVV check failed";
        }
        else
        {
//            if($p->credit(false) == false)
//                $error = "<b>refund failed<b>: " . $p->getLastMessage();
//            else
*/
                $result = $p->getResult();
                //$error = "Refunded. Spend your money elsewhere...";
        //}
    }
...
//print_r($result); exit;
    ...
    if ($result["RESPMSG"] == $OKSTATUS || $result["RESPMSG"] == $OKSTATUS1) $result["RESPMSG"] = 'Completed';

    $tel="";
    $name = "$payment->mem_surname $payment->mem_forename";

    $payment = save_payment_details($payment->pay_paymentid, $result['PNREF'], $result["RESPMSG"], date('Y-m-d H:i'), $name, $payment->mem_email, $zip, $country, $address1, $tel, $address_country, 'payflow_api',array('request'=>$order,'responce'=>$result));
    if ($error != '') {
...
<table width="<?php print("$CONST_TABLE_WIDTH"); ?>" align="<?php print("$CONST_TABLE_ALIGN"); ?>" border="0" cellspacing="<?php print("$CONST_TABLE_CELLSPACING"); ?>" cellpadding="<?php print("$CONST_TABLE_CELLPADDING"); ?>">
  ...
  <tr>
    ...
    <tr>
        ...
        <td valign=middle>
            <select name="cardtype" class=inputf>
                <option value="">
...
<?/*                <option value="Australian BankCard">Australian BankCard*/?>
    ...
    <tr>
        ...
        <td><input type="text" name="cardnumber" value="" class=input></td>
    </tr>
    <tr>
        <td width=30%>*Expire date</td>
        <td>
            <select name="cardexpmonth" class=inputf>
                <option value="">Month
<?for ($i=1;$i<=12;$i++){?>
                <option value="<?=sprintf('%02.2s',$i)?>"><?=sprintf('%02.2s',$i)?>
<?}?>
            </select>
            <select name="cardexpyear" class=inputf>
                <option value="">Year
...
<?}?>
    ...
    <tr>
        ...
        <td><input type="text" name="cvmvalue" value="" size=4 maxlength=4 class=input></td>
    </tr>
    <tr>
        <td width=30%>Name</td>
    ...
    <tr>
        ...
        <td><input type="text" name="address1" value="" class=input></td>
    </tr>
    <tr>
        <td width=30%>State</td>
        <td>
            <select name="state" class=inputf>
            <option value="">Select
    ...
    <tr>
        <td width=30%>*Zip</td>
        <td><input type="text" name="zip" value="" class=input></td>
    </tr>
    <tr>
