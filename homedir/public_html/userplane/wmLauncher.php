        ...
        if( $userID )
        {
            ...
            while( $rsArray = mysql_fetch_array( $rs ) )
            {
                ...
                if ($option_manager->GetValue('userplane_im')){
                    echo( "window.parent.up_launchWM( '" . $userID . "', '" . $rsArray["originatingUserID"] . "', '" . $rsArray["displayName"] . "' );\n" );
                } elseif ($option_manager->GetValue('userplane_im_free')) {
                    echo( "window.parent.up_launchWM_free( '" . $userID . "', '" . $rsArray["originatingUserID"] . "', '" . $rsArray["displayName"] . "' );\n" );
                }
//                                echo( "window.parent.up_launchWM( '" . $userID . "', '" . $rsArray["originatingUserID"] . "', '" . $rsArray["displayName"] . "' );\n" );
                // if you cannot get their names, adjust the query above and use the following line:
                // echo( "window.parent.up_launchWM( '" . $userID . "', '" . $rsArray["originatingUserID"] . "' );\n" );
            }
            echo( "//--></script>" );
        }
