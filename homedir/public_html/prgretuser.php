...
$conStingru = @mysqli_connect(__CONST_DB_HOST, __CONST_DB_USER, __CONST_DB_PASS, __CONST_DB_NAME);

$result = mysqli_query($conStingru, "
...
if ($userid != $Sess_UserId) {

    $query = "SELECT * FROM encounters WHERE enc_userid = '$userid' AND enc_viewerid='$Sess_UserId'";

    $result = mysqli_query($conStingru, $query) or die(mysql_error());
    ...
    if ($TOTAL == 0) {
        ...
        $result = mysqli_query($conStingru, "INSERT INTO encounters (enc_userid, enc_viewerid, enc_viewdate) VALUES ('$userid','$Sess_UserId','$tempdate')") or die(mysqli_error());
    } else {

        $tempdate = date("Y-m-d");

        $result = mysqli_query($conStingru, "UPDATE encounters SET enc_viewdate='$tempdate' WHERE enc_userid = '$userid' AND enc_viewerid='$Sess_UserId'") or die(mysqli_error());
    }
}
...
$result = mysqli_query($conStingru, "SELECT * FROM mymatch WHERE mym_userid=$Sess_UserId") or die(mysqli_error());
...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($sql_me->mym_gender == $sql_array->adv_sex)
        $score += 50;

    if (($sql_array->age >= $sql_me->mym_agemin) && ($sql_array->age <= $sql_me->mym_agemax))
        $score += 10;

    if (($sql_array->adv_height >= $sql_me->mym_minheight) && ($sql_array->adv_height <= $sql_me->mym_maxheight))
        $score += 10;

    if (($sql_array->adv_bodytype == $sql_me->mym_bodytype) OR ($sql_me->mym_bodytype == 'Not stated'))
        $score += 10;

    if (($sql_array->adv_seeking == $sql_me->mym_relationship) OR ($sql_me->mym_mym_relationship == 'Not stated'))
        $score += 10;

    if (($sql_array->adv_smoker == $sql_me->mym_smoker) OR ($sql_me->mym_smoker == 'Not stated'))
        $score += 10;

    if ($score < 60)
        $myscore = PRGRETUSER_POOR;

    elseif ($score < 80)
        $myscore = PRGRETUSER_FAIR;

    elseif ($score < 100)
        $myscore = PRGRETUSER_GOOD;
} else {
...
$result = mysqli_query($conStingru, "SELECT * FROM mymatch WHERE mym_userid=$userid") or die(mysqli_error());

if (mysqli_num_rows($result) > 0) {

    $sql_they = mysqli_fetch_object($result);

    if ($sql_they->mym_gender == 'M')
        $mygender = PRGSTATS_MALES;

    elseif ($sql_they->mym_gender == 'F')
        $mygender = PRGSTATS_FEMALES;

    elseif ($sql_they->mym_gender == 'C')
        $mygender = PRGSTATS_COUPLE;

    $result = mysqli_query($conStingru, "SELECT (YEAR(CURDATE())-YEAR(adv_dob)) - (RIGHT(CURDATE(),5) < RIGHT(adv_dob,5)) AS age, adv_height, adv_bodytype, adv_seeking, adv_sex, adv_smoker FROM adverts WHERE adv_userid=$Sess_UserId") or die(mysqli_error());
    ...
    if ($sql_they->mym_gender == $sql_me->adv_sex)
        $score += 50;

    if (($sql_me->age >= $sql_they->mym_agemin) && ($sql_me->age <= $sql_they->mym_agemax))
        $score += 10;

    if (($sql_me->adv_height >= $sql_they->mym_minheight) && ($sql_me->adv_height <= $sql_they->mym_maxheight))
        $score += 10;

    if (($sql_me->adv_bodytype == $sql_they->mym_bodytype) OR ($sql_they->mym_bodytype == 'Not stated'))
        $score += 10;

    if (($sql_me->adv_seeking == $sql_they->mym_relationship) OR ($sql_they->mym_relationship == 'Not stated'))
        $score += 10;

    if (($sql_me->adv_smoker == $sql_they->mym_smoker) OR ($sql_they->mym_smoker == 'Not stated'))
        $score += 10;

    if ($score < 60)
        $theyscore = PRGRETUSER_POOR;

    elseif ($score < 80)
        $theyscore = PRGRETUSER_FAIR;

    elseif ($score < 100)
        $theyscore = PRGRETUSER_GOOD;
} else {
...
if (isset($Sess_UserId)) {
    ...
    include("$CONST_INCLUDE_ROOT/skype.inc.php");
    ?>
    ...
    <?php } ?>
                                                    ...
                                                    <?php
                                                    $blkquery = mysqli_query($conStingru, "SELECT * FROM blockmail WHERE blk_receiverid = $userid AND blk_senderid = $Sess_UserId");
    ...
    <?php
    $advQuery = mysqli_query($conStingru, "SELECT * FROM adverts where adv_userid=$Sess_UserId AND adv_approved=1");

    $advNote = mysqli_num_rows($advQuery);

    if ($CONST_FLIRT == 'Y' && $advNote > 0) {

        $NoteQuery = mysqli_query($conStingru, "SELECT * FROM notifications where ntf_senderid=$Sess_UserId and ntf_receiverid=$userid");
                        ...
                        <table width="70%" border="0" cellspacing="0" cellpadding="4">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div style="overflow:hidden; width:<?= $CONST_COMMENT_WIDTH - 50 ?> px;">

<?php print $sql_array->adv_comment_full; ?></div></td>
...
<?php if ($option_manager->GetValue('snetwork')) { ?><?php include_once "$CONST_NETWORK_INCLUDE_ROOT/action.inc.php" ?><?php } ?>

                                    <table width="98%" border="0" align="center" cellpadding="2" cellspacing="0">

                                        <tr><td>

<?php if ($option_manager->GetValue('blogs')) { ?>
...
<?php echo stripslashes(nl2br($sql_they->mym_comment)) ?>
...
if (can_navigate_back()) {
    print (" | <a href=\"" . get_prev_page_url() . "\">Back</a>");
}
?>
