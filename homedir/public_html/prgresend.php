...
if ($retval < 1) {
	...
	if($sql_array->mem_confirm == 0){

		$data['ConfirmLink'] = $CONST_LINK_ROOT."/confirm.php?id=".md5($sql_array->mem_userid);

        $template = 'Confirm_Resend';

	} else {

        $template = 'Login_Resend';

	}



    $data['Login']        = $sql_array->mem_username;

    $data['Password']     = $sql_array->mem_password;

    $data['SupportEmail'] = $CONST_SUPPMAIL;

    $data['Url']          = $CONST_URL;

    list($type,$message) = getTemplateByName($template,$data,getDefaultLanguage($sql_array->mem_userid));
	...
	send_mail ($txtEmail, $CONST_MAIL, PRGRESEND_TEXT3, $message, $type, "ON");
...
<?=$skin->ShowHeader($area)?>
  ...
  <table width="<?php print("$CONST_TABLE_WIDTH"); ?>" align="<?php print("$CONST_TABLE_ALIGN"); ?>" border="0" cellspacing="<?php print("$CONST_TABLE_CELLSPACING"); ?>" cellpadding="<?php print("$CONST_TABLE_CELLPADDING"); ?>">
    ...
    <td><?php echo PRGRESEND_TEXT1?> <p><a href="<?php echo $CONST_LINK_ROOT?>/login.php"><?php echo GENERAL_CONTINUE?></a></p></td>
