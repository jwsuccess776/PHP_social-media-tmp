...
class GroupPost extends Main {
    ...
    public $strictIntegrityCheck;
    public $integrityFlaws = array();

    function GroupPost($id = 0, $strictIntegrityCheck = true) {
        ...
        if (is_array($data)) {
            ...
            if (!empty($data['groupid']) && intval($data['groupid'])) {
                if ($db->count('groups', "id = '".$this->_prepareData($data['groupid'])."'"))
                    ...
                    $this->CriticalError('Topic to Group integrity fault');
            } elseif (!$this->groupid) // group ID wasn't prepopulated
                $this->CriticalError('No Group to post to');
            ...
            if (!empty($data['author']) && intval($data['author'])) {
                ...
                $this->CriticalError('No Author for post');
            }
    ...
    function initDraftByArray($data) {
        if (is_array($data)) {
            ...
            if (!empty($data['id']) && intval($data['id']))
                ...
                $this->CriticalError('Draft has no ID');
            ...
            if (!empty($data['groupid']) && intval($data['groupid'])) {
                ...
                $this->CriticalError('Draft has no Group');
            ...
            if (!empty($data['author']) && intval($data['author'])) {
                ...
                $this->CriticalError('No Author for draft');
            }
