<?php
class ScmsImage extends canvasCrop
{
    function scmsImage($debug = FALSE)
    {
        $this->canvasCrop($debug);
    }

    function applyChangesToOriginal() {
         $this->_imgOrig = $this->_imgFinal;
    }

    function getInfo() {
        return array( @ImageSX($this->_imgOrig) , @ImageSY($this->_imgOrig) );
    }

    function redimToPercent($percentx, $percenty, $crop = FALSE)
    {
        $percentx = ($percentx > 100)?100:$percentx;
        $percenty = ($percenty > 100)?100:$percenty;
        $x = (($percentx / 100) * @ImageSX($this->_imgOrig));
        $y = (($percenty / 100) * @ImageSY($this->_imgOrig));
        return ($this->_reSize($x, $y, $crop, 'redimToPercent'));
    }
...
class canvasCrop
{
    ...
    */
    function cropToDimensions($sx, $sy, $ex, $ey)
    {
        $sx = ($sx < 0)?0:$sx;
        $sy = ($sy < 0)?0:$sy;
        $ex = ($ex > @ImageSX($this->_imgOrig))?@ImageSX($this->_imgOrig):$ex;
        $ey = ($ey > @ImageSY($this->_imgOrig))?@ImageSY($this->_imgOrig):$ey;
        $nx = abs($ex - $sx);
        $ny = abs($ey - $sy);
    ...
    */
    ...
    {
        /* caused bugs. replaced with file type check.
        ...
        */
        ...
        if (file_exists($filename)) { // check file type for existing file 
            $info = getimagesize($filename);
            $ext = empty($extensions[$info[2]]) ? '' : $extensions[$info[2]];
        } else { // parse filename for extension
