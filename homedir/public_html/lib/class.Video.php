...
class Video extends ratedItem {
    ...
    var $filename;
    var $filepath;
    var $allowed_status = array('new', 'rejected', 'converted');
    var $rating_name = 'video';
    var $vid_views;
    /**
     * Constructor
     *
...
//        if ($data['filepath'] == '') return $this->Error("File is empty. Please input file for uploading");
    ...
    function Delete($user_id, $admin = false){
        ...
        if ($row = $db->get_row("SELECT * FROM videos WHERE vid_id = '$this->vid_id' $owner_filter")){
            ...
            $this->rating->deleterate($this->vid_id);
    ...
    function convert(){
        ...
        extension_loaded($extension_soname) or $this->criticalError("Can't load extension $extension_fullname\n");
        ...
        if (!is_object($frameSnap)) return $this->Error("Can't get frame. Maybe video file is corrupted.");
