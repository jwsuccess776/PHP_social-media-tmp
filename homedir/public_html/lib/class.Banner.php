...
class Banner extends Main {
    ...
    function getList($size, $order = 'bc.size, bannerName', $page = 0) {
        $db =& db::getInstance();
    ...
    function save($size, $label, $code, $hits, $active) {
        ...
        if (!$code) return $this->Error('Empty code');
        if (!$hits) $hits = -1;
        if ($active) $active = 1;
        if (!$this->id) {
    ...
    function displayBanner($size, $print = true, $count = true) {
        $db =& db::getInstance();
        $ids = $db->get_col("SELECT banner_id 
                              ...
                              WHERE size = '$size' &&
                                    is_active = 1 &&
                                    hits != 0");
        if (!isset($ids)) return null; // no banners to display
