...
include('db_connect.php'); include_once('validation_functions.php');
include('session_handler.inc');
include('error.php');

if ($_GET['del']==1){
    if ($_GET['event_id']){
        $query = "SELECT * FROM events WHERE ev_eventid=".$_GET['event_id'];
        $res= mysql_query($query);
        $row = mysql_fetch_object($res);
        mysql_query("DELETE FROM events WHERE ev_eventid=".$_GET['event_id']);
        if ($row->ev_picture) unlink($CONST_INCLUDE_ROOT."/".$row->ev_picture);
        mysql_query("DELETE FROM reviews WHERE review_id=".$_GET['event_id']);
    }
...
$countries = mysql_query($query,$link) or die(mysql_error());
$query = "
        SELECT * FROM geo_state
        WHERE gst_stateid > 0
        AND gst_countryid = '$lstCountry'
        ORDER BY gst_name";
$states = mysql_query($query,$link) or die(mysql_error());
if (isset($lstState)) {
      $query = "
        SELECT * FROM geo_state
        ...
        AND gst_countryid = '$lstCountry'
        ORDER BY gst_name";
      $checkstate = mysql_query($query,$link) or die(mysql_error());
      if (mysql_num_rows($checkstate) == 0) unset($lstState);
}
if (isset($lstState)) {
      $query = "
        ...
        ORDER BY gct_name";
      $cities = mysql_query($query,$link) or die(mysql_error());
} else {
      $query = "
        SELECT * FROM geo_city
        WHERE
...
<table width="<?php print("$CONST_TABLE_WIDTH"); ?>" align="<?php print("$CONST_TABLE_ALIGN"); ?>" border="0" cellspacing="<?php print("$CONST_TABLE_CELLSPACING"); ?>" cellpadding="<?php print("$CONST_TABLE_CELLPADDING"); ?>">
  ...
  <tr>
    <td>
        <form enctype='multipart/form-data' action="<?php echo $CONST_LINK_ROOT ?>/events.php" method='post' name="FrmEvent" id="FrmEvent">
...
-->
                <select class="input" name="lstCountry" id="lstCountry" size="1" tabindex="1" onChange="FrmEvent.mode.value='view'; submit(); return true;">
                    ...
                    <option value=""></option>
                <?php
                        while ($sql_array = mysql_fetch_object($countries)) {
                ...
                <select class="input" name="lstState" id="lstState" size="1" tabindex="1" onChange="FrmEvent.mode.value='view'; submit(); return true;">
                    <option value="0" selected><?php echo SEARCH_ANY?></option>
                <?php
                        while ($sql_array = mysql_fetch_object($states)) {
                ...
                <select class="input" name="lstCity" id="lstCity" size="1" tabindex="1" onChange="FrmEvent.mode.value='view'; submit(); return true;">
                    <option value="0" selected><?php echo SEARCH_ANY?></option>
                <?php
                        while ($sql_array = mysql_fetch_object($cities)) {
