...
if (isset($_POST['adchangevalue']) && $_POST['adchangevalue'] != '') {
	$adchangevalue=sanitizeData($_POST['adchangevalue'], 'xss_clean') ;   
...
if ($Sess_Userlevel!="silver") {
	...
	//$sql_array = mysql_fetch_array($result);
        $sql_array=$db->get_results($query);
...
if (formGet('HIDE_FEATURED')) {
    $value = $db->escape(formGet('hidefeatured'));
	$db->query("UPDATE members SET mem_featured='$value' WHERE mem_userid = '$Sess_UserId'");
}
...
$query="SELECT adv_userid,adv_picture, adv_paused, adv_approved,mem_block_mail,mem_sms, mem_featured 
		FROM adverts
...
<?=$skin->ShowHeader($area)?>
  <table width="<?php print("$CONST_TABLE_WIDTH"); ?>" align="<?php print("$CONST_TABLE_ALIGN"); ?>" border="0" cellspacing="<?php print("$CONST_TABLE_CELLSPACING"); ?>" cellpadding="<?php print("$CONST_TABLE_CELLPADDING"); ?>">
    ...
    <td width="60%" class="pageheader"><?php echo MYINFO_SECTION_NAME ?></td>
    <td align="right" width="40%" valign="top" rowspan="2"><img src="<?php echo $CONST_LINK_ROOT; ?>/images/my-info-image.jpg" /></td>
    </tr>
    <tr>
        <td>
        ...
        <form  name="frmMyinfo" method="post" action="<?=$CONST_LINK_ROOT?>/myinfo.php">
          <tr>
            <td align="left" valign="top" nowrap class="tdodd"><strong><?=HIDE_FEATURED?></strong></td>
            <td align="left" valign="top" class="tdodd">
				<select name="hidefeatured">
				<option value="0" <?if ($got_photo->mem_featured == 0){?>selected<?}?>> <?=GENERAL_YES?>
				<option value="1" <?if ($got_photo->mem_featured == 1){?>selected<?}?>> <?=GENERAL_NO?>
				</select> <input type=submit name=HIDE_FEATURED  class="button" value="<?=MYINFO_SAVE?>">
			</td>
          </tr>
        </form>
